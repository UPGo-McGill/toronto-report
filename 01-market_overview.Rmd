...
Active daily listings are listings which were displayed on Airbnb or VRBO on a given day, and were either reserved or available for a reservation. It is the most reliable means of determining the overall size of the STR market in a location, particularly with respect to change over time. In 2020 there was an average of `r avg_listings_active` active daily listings (Figure \@ref(fig:fig-1-1)) operated by an average of `r avg_hosts` hosts. These hosts collectively earned `r annual_rev`—an average of `r avg_rev_per_listing` per daily active listing or `r avg_rev_per_host` per active host. The 2020 STR market has significantly been impacted by the Covid-19, both because of the broad decline in travel demand and of Airbnb's announcement of an emergency cancellation policy which enabled guests to cancel their reservations if the reservation start date was between March 14 and July 31, 2020 (Airbnb 2020). To compare, there was an average of `r avg_listings_active19` active daily listings in 2019, operated by `r avg_hosts19` hosts, who earned on average `r avg_rev_per_host19`. 
...
Active daily listings peaked in `r highest_activity_date` at `r highest_activity_amount`, and have since declined. Due to the Covid-19 pandemic, there were `r active_yoy_listings` less active listings on average in 2020 than in 2019, and `r active_yoy_revenue` less revenue. Prior to 2020, Toronto's STR market was growing at a great rate. Compared to 2018, active listings and revenue respectively increased by `r active_yoy_listings19` and `r active_yoy_revenue19` in 2019.
...
Table \@ref(tab:tab-1-3) provides the distribution of the listings by listing type in 2019. The majority of STRs in Toronto are entire homes, a category which includes single-family homes, townhouses, apartments and condominiums. Half (`r one_bedrooms`) of these were one-bedroom housing units, and more than a quarter (`r two_bedrooms`) were two-bedrooms units. `r three_bedrooms` were 3 or more bedrooms housing units, and `r studios` were studios. In general, studios and one-bedroom units are overrepresented on STR platforms in comparison with the City’s overall housing stock.
...
When it comes to reserved and available listings, reservations generally spike during the winter holidays, but otherwise are low from November to February, and then increase steadily from March to August before reclining again. Available nights follow a roughly opposite pattern, since availability is higher when fewer reservations are made. Figure \@ref(fig:fig-5-1) shows the number of STR listings available and reserved each day in Toronto. While the total reserved nights increased by a rapid `r reserved_nights_increase_jan_feb` in the first two months of 2020, they have overall decreased by `r reserved_nights_decrease_2020` in 2020 compared to 2019. There was `r reservation_change_2020` fewer nights reserved in 2020, from `r reservation_count_2020[1]` reserved nights in 2019 to `r reservation_count_2020[2]` reserved nights in 2020. 
...
trends <-
  tibble(
    date = as.Date(c("2020-03-16", "2020-04-16", "2020-05-16", "2020-06-16",
                     ...
                     "2020-10-31")),
    trend = (feb_trend + mar_nov_seasonal) / 
      c(31, 30, 31, 30, 31, 31, 30, 31, 30))

# Set Oct 31 value to average of October and November
trends[9,]$trend <- mean(trends[8:9,]$trend)
...
# Get March-November seasonal
mar_nov_price_seasonal <- 
  monthly_prices %>% 
  filter(yearmon <= tsibble::yearmonth("2020-02")) %>% 
...
mar_nov_price_trend <- 
  tibble(
    date = as.Date(c("2020-03-16", "2020-04-16", "2020-05-16", "2020-06-16",
                     "2020-07-16", "2020-08-16", "2020-09-16", "2020-10-16",
                     "2020-10-31")),
    trend = (feb_price_trend + mar_nov_price_seasonal))

# Set Oct 31 value to average of October and November
mar_nov_price_trend[9,]$trend <- mean(mar_nov_price_trend[8:9,]$trend)

# Apply to daily averages to get trend
average_prices <- 
  average_prices %>% 
  mutate(yearmon = tsibble::yearmonth(date)) %>% 
  inner_join(mar_nov_price_trend) %>% 
  group_by(yearmon) %>% 
...
In terms of revenue, 2020 revenue decreased by a staggering `r active_yoy_revenue` compared to 2019. To offer some perspective, from 2018 to 2019, STR revenue increased by `r active_yoy_revenue`. Based on previous growth trends, it is possible to estimate the total loss in revenue incurred by STR hosts, combining the loss of revenue of reservations that never occurred and the lower nightly price of reservations that did occur. Throughout the May-October period of 2020, nightly prices have been an average of `r nightly_price_dif` lower than expected. Spread across the `r res_total_pandemic` nights reserved during this period, this means that STR operators collectively earned `r total_price_dif` less than they would have on their bookings in the absence of the pandemic. When the lower prices on reservations which did occur is combined with the reservations which did not occur, our estimate is that Toronto’s STR hosts lost a total of `r total_revenue_lost` million in revenue between March and October 2020 because of the Covid-19 pandemic.
...
In 2020, `r ml_listings` of active listings in Toronto were multilistings, earning `r ml_revenue` of total host revenue. Multilistings had been growing steadily since 2017, both in terms of listings and revenue percentage, until the Covid-19 pandemic, where there was first a drop of revenue percentage, later followed by a drop in multilistings percentage (Figure \@ref(fig:fig-1-5)). The share of multilistings in both listings share and revenue caught up during the summer of 2020. Figure \@ref(fig:fig-1-5) also highlights that, amidst generally declining STR activity during the COVID-19 pandemic, multilistings have remained strong, and through the fall of 2020 were earning a bit more than 4 out of every 10 dollars on STR platforms in Toronto. 
...
(ref:fig-1-5) _The percentage of active listings and revenue accounted for by multilistings in Toronto (14-day average)_
