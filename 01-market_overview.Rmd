...
waterfront_listing_pct <- 
  WD_breakdown %>% 
  filter(ward == "Waterfront Communities-The Island") %>% 
  pull(listings_pct) %>% 
  scales::percent(0.1)
...
church-yonge_listing_pct <- 
  WD_breakdown %>% 
  filter(ward == "Church-Yonge Corridor") %>% 
  pull(listings_pct) %>% 
  scales::percent(0.1)

church-yonge_rev_pct <- 
  WD_breakdown %>% 
  filter(ward == "Church-Yonge Corridor") %>% 
  pull(rev_pct) %>% 
  scales::percent(0.1)
...
downtown_per_cap <- 
  WD_breakdown %>% 
  filter(ward == "Downtown") %>% 
  pull(listings_pct) %>% 
  scales::percent(0.1)

riley_per_cap <- 
  WD_breakdown %>% 
  filter(ward == "Riley Park") %>% 
  pull(listings_pct) %>% 
  scales::percent(0.1)
...
STR activity in Toronto is highly concentrated in the Spadina-Fort York ward, which includes the Waterfront area (Figure \@ref(fig:fig-1-3)). This ward accounts for `r downtown_listing_pct` of all listings in 2019, and an even higher share of host revenue (`r downtown_rev_pct`). Spadina-Fort York is followed by TKTK (`r kits_listing_pct` of active listings and `r kits_rev_pct` of revenue) and the TKTK ward (`r west_listing_pct` of active listings and `r west_rev_pct` of revenue). When measured in per-capita terms, Spadina-Fort York and TKTK have the highest concentrations of STRs, at `r downtown_per_cap` and `r riley_per_cap` respectively. In all other wards STRs account for between `r min_per_cap` and `r max_per_cap` of total dwellings.
...
While current STR activity is more concentrated Spadina-Fort York, new STR growth is occurring in other wards. In Table \@ref(tab:tab-1-2), entries are orange if the ward's value is higher than the City average, and blue if it is lower (TKTK NOT CURRENTLY IMPLEMENTED). The table reveals an overall inverse relationship between current STR activity and growth trends; while for example Downtown accounts for a plurality of existing listings and revenue, both listing and revenue growth is substantially higher in outlying wards (Marpole and DuWDar-Southlands, for example). This pattern points to an ongoing decentralization of STR activity in Toronto. 
...
figure_1_4_fun <- function(regular = "", condensed = "") {
  ...
  geom_ward(colour = "white", lwd = 1.2) +
