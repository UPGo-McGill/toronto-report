...
```{r new_objects, cache=TRUE, cache.extra=, cache.lazy=FALSE}
...
active_2020 <- 
  daily %>%
  filter(housing, status %in% c("R", "A"), date <= LTM20_end_date, 
         date >= LTM20_start_date) %>%
  pull(property_ID) %>% 
  unique()
...
revenue_2020 <-
  daily %>%
  filter(housing, status == "R", date <= LTM20_end_date, 
         date >= LTM20_start_date) %>%
  group_by(property_ID) %>%
  summarize(revenue_LTM20 = sum(price), .groups = "drop") %>% 
  inner_join(property, ., by = "property_ID")
...
host_rev20 <-
  daily %>%
  filter(housing, date >= LTM20_start_date, date <= LTM20_end_date, 
         status == "R", !is.na(host_ID)) %>%
  group_by(host_ID) %>%
  summarize(rev = sum(price))
...
avg_listings_active20 <- 
  avg_listings20$avg[1] %>% 
  prettyNum(",")
...
ml_listings20 <- 
  daily %>% 
  filter(housing, status != "B", date >= LTM20_start_date, 
         date <= LTM20_end_date) %>% 
  count(multi) %>% 
  summarize(ML = n[2] / sum(n)) %>% 
  pull(ML) %>% 
...
ml_revenue20 <- 
  daily %>% 
  filter(housing, status == "R", date >= LTM20_start_date, 
       date <= LTM20_end_date) %>% 
  group_by(multi) %>% 
  tally(price) %>% 
  summarize(multi_rev = n[2] / sum(n)) %>% 
...
**There were `r avg_listings_active` active STR listings in Toronto housing units in 2019, which collectively earned `r annual_rev`. A quarter of all listings are in the Downtown ward. More than two thirds (`r eh_listings`) of Toronto’s STR listings are entire homes, and entire homes are responsible for `r eh_revenue` of all revenue. Revenue is concentrated unevenly among hosts, with the top 10% earning `r host_rev_top_10` of revenue, and the top 1% earning `r host_rev_top_1`, but Toronto has much less revenue concentration than other large cities in Canada. Commercial operators who control multiple STR listings account for `r ml_listings` listings and `r ml_revenue` revenue, and those shares have been falling consistently since the introduction of the City's 2018 STR regulations.**
...
figure_1_1_fun <- function(regular = "", condensed = "") {
  ...
  annotate("segment", x = key_date_regulations, xend = key_date_regulations,
           y = -Inf, yend = Inf, alpha = 0.3) +
  annotate("curve", x = as.Date("2020-10-01"), xend = key_date_regulations - days(10),
           y = 5200, yend = 5100, curvature = -.2, lwd = 0.25,
           arrow = arrow(length = unit(0.05, "inches"))) +
  annotate("text", x = as.Date("2020-08-01"), y = 5200,
...
Active daily listings are listings which were displayed on Airbnb or VRBO on a given day, and were either reserved or available for a reservation. It is the most reliable means of determining the overall size of the STR market in a location, particularly with respect to change over time. In 2019 there was an average of `r avg_listings_active` active daily listings (Figure \@ref(fig:fig-1-1)) operated by an average of `r avg_hosts` hosts. These hosts collectively earned `r annual_rev`—an average of `r avg_rev_per_listing` per daily active listing or `r avg_rev_per_host` per active host. In 2020 the daily active listings average declined to `r avg_listings_active20` and were operated by an average of `r avg_hosts20` hosts. These hosts collectively earned `r annual_rev20`—an average of `r avg_rev_per_listing20` per daily active listing or `r avg_rev_per_host20` per active host.
...
avg_listings_blocked20 <- 
  avg_listings20$avg[2] %>% 
  prettyNum(",")
...
There was also a daily average of `r avg_listings_blocked` listings which were visible on the Airbnb and VRBO websites but were blocked by the host from receiving reservations (`r avg_listings_blocked` in 2020). The presence of these listings can erroneously suggest that a city’s STR market is larger than it is. When these blocked but inactive listings are included, the average listing earned `r avg_revenue_all_listings` in 2019, and the average host earned `r avg_revenue_all_hosts`. Finally, there was a daily average of `r non_housing_listings` listings that were not located in private housing units (B&Bs, hotels, etc.) (`r non_housing_listings20` in 2020), which have been excluded from the analysis in this report. All the figures which follow, therefore, pertain to short-term rentals located in Toronto’s traditional residential housing stock.
...
active_yoy_change20 <- 
  daily %>% 
  filter(housing, status != "B", date >= LTM20_start_date - years(1), 
         date <= LTM20_end_date) %>% 
  group_by(year_2020 = date >= LTM20_start_date) %>% 
  summarize(active_listings = n() / 365,
            revenue = sum(price[status == "R"]), .groups = "drop") %>% 
  summarize(across(c(active_listings, revenue), ~{(.x[2] - .x[1]) / .x[1]}),
            .groups = "drop")

active_yoy_listings20 <- 
  active_yoy_change20$active_listings %>% 
  {. * -1} %>% 
  scales::percent(accuracy = 0.1)

active_yoy_revenue20 <- 
  active_yoy_change20$revenue %>% 
  scales::percent(accuracy = 0.1)
...
Active daily listings peaked in `r highest_activity_date` at `r highest_activity_amount`, and have since declined. There were  `r active_yoy_listings` more listings active on average in 2019 than in 2018, and `r active_yoy_revenue` more revenue. Due to the COVID-19 pandemic, active listings decreased by `r active_yoy_listings20` in 2020 and revenues plummeted, with `r active_yoy_revenue20` less revenue earned during the year, compared to 2019.

In general, the number of STR listings available in the City of Toronto peaks during late summer and winter holidays and falling in between these periods (Figure \@ref(fig:fig-1-1)). There is one major exception the March to December 2020 period, where activity has been sharply below the seasonal trend because of the Covid-19 pandemic.
...
listing_growth_2020 <- 
  ...
  filter(housing, status != "B", date >= LTM_start_date,
         date <= LTM20_end_date,
         (date <= max(daily$date) - years(1) | date >= LTM20_start_date),
         date != "2020-02-29") %>%
  group_by(year_2020 = date >= LTM20_start_date) %>% 
...
Overall, the year-over-year change in average active listings from 2016 to 2017 (12 months) was `r listing_growth_2017`, the year-over-year change from 2017 to 2018 was `r listing_growth_2018`, and the year-over-year change from 2018 to 2019 was `r listing_growth_2019`. In 2020, active listings fell thanks to the Covid-19 pandemic. The year-over-year change in active daily listings for 2020 so far (January to `r end_month`) is `r listing_growth_2020`.
...
As for the growth rate of reservations and host revenue, in 2019, the number of reserved nights increased by `r reservation_change_2019`, from `r reservation_count_2019[1]` reserved nights in 2018 to `r reservation_count_2019[2]` reserved nights in 2019, while revenue increased `r active_yoy_revenue`. Due to the Covid-19 pandemic, revenue from January to `r end_month` 2020 is down `r active_yoy_revenue_2020` compared to the same time last year.
...
In 2019, Toronto had the largest STR market in the country by both active listing numbers (`r avg_listings_active`) and host revenue (`r annual_rev`), followed in both cases by Montreal and Vancouver (Table \@ref(tab:tab-1-1)). However, in relative terms, Vancouver stands considerably ahead of both Toronto and Montreal. Vancouver had the most active listings per 1000 households (`r van_active_per_1000` compared to `r to_active_per_1000` in Toronto) and the most revenue per listing (`r van_rev_per_listing` compared to `r to_rev_per_listing` in Toronto).
...
STR activity in Toronto is highly concentrated in the Spadina-Fort York ward, which includes the Waterfront area (Figure \@ref(fig:fig-1-3)). This ward accounts for `r spadina_listing_pct` of all listings in 2019, and an even higher share of host revenue (`r spadina_rev_pct`). Spadina-Fort York is followed by the Toronto Centre ward (`r centre_listing_pct` of active listings and `r centre_rev_pct` of revenue) and the TKTK ward (`r west_listing_pct` of active listings and `r west_rev_pct` of revenue). When measured in per-capita terms, Spadina-Fort York and TKTK have the highest concentrations of STRs, at `r spadina_per_cap` and `r riley_per_cap` respectively. In all other wards STRs account for between `r min_per_cap` and `r max_per_cap` of total dwellings.
...
(ref:fig-1-3) _Active STRs as a share of all dwelling units in Toronto, by local ward (L) and dissemination area (R)_

TKTK REVISIT WITH PROCESSED DATAWhile current STR activity is more concentrated Spadina-Fort York, new STR growth is occurring in other wards. In Table \@ref(tab:tab-1-2), entries are orange if the ward's value is higher than the City average, and blue if it is lower (TKTK NOT CURRENTLY IMPLEMENTED). The table reveals an overall inverse relationship between current STR activity and growth trends; while for example Downtown accounts for a plurality of existing listings and revenue, both listing and revenue growth is substantially higher in outlying wards (Marpole and Dunbar-Southlands, for example). This pattern points to an ongoing decentralization of STR activity in Toronto. 
...
WD_breakdown %>% 
  ...
  kbl(caption = "Local wards with at least 100 daily active listings in 2019",
               align = "lrrrrr") %>% 
  kable_styling(latex_options="scale_down") %>% 
...
In 2019 entire-home listings accounted for `r eh_listings` of all daily active listings, and `r eh_revenue` of total host revenue. (Private rooms accounted for nearly all of the remainder.) Moreover, the dominance of entire-home listings in Toronto’s STR market is increasing over time; while there were fewer active listings among all listing types in 2019 when compared to 2018, active daily entire-home listings declined by a much smaller proportion than the other listing types.
...
top_host_listings20 <- 
  property %>% 
  filter(host_ID == host_rev[which.max(host_rev20$rev),]$host_ID) %>% 
  nrow()
...
Among all the STR hosts who earned revenue in the City of Toronto in 2019, the median revenue was `r median_host_rev` (Table \@ref(tab:tab-host-rev)). Throughout the City of Toronto, there were `r n_hosts_over_250` hosts that earned more than $250,000 in 2019. Figure \@ref(fig:fig-1-4) shows the percentage of the total `r annual_rev` in STR revenue which accrued to each decile of hosts. The figure shows that revenue is disproportionately concentrated among a small number of hosts; the top 10% earned `r host_rev_top_10`: the top 5% earned `r host_rev_top_5` of revenue, while the top 1% of hosts earned `r host_rev_top_1` of all revenue. To put this revenue distribution in context, Montreal’s top 10% of STR hosts earned 68.8% of all revenue in 2019. One the other hand, Vancouver's top 10% hosts only earned 43.2% of all revenue in 2019. This indicates that Toronto's STR market is dominated by commercial operators compared to its Western peer, but to a lesser extent than Montreal.
...
host_rev %>% 
  ...
  kbl(caption = "Toronto STR host revenue", align = "lr") %>% 
  kable_styling()
...
(ref:fig-1-4) _STR host revenue distribution in the City of Toronto_
...
In 2019, `r ml_listings` of active listings in Toronto were multilistings, earning `r ml_revenue` of total host revenue. Multilistings had been growing steadily since 2017, both in terms of listings and revenue percentage, until Airbnb’s mass removal of listings without licence numbers. Since then, the proportion of multilistings and their share of overall revenue has been decreasing. Multilisting revenue as a share of total revenue reached an all-time high right before Airbnb's August 2018 regulatory enforcement takedown, while multilisting active listings as a share of total active listings peaked several months later (Figure \@ref(fig:fig-1-5)). Both indicators have been in relatively steady decline since then, and by the end of 2019 were considerably lower than in any of the other large cities in Canada. Figure \@ref(fig:fig-1-5) also highlights that, amidst generally declining STR activity during the COVID-19 pandemic, multilistings have declined disproportionately quickly, and through the fall of 2020 were earning a bit more than 1 out of every 4 dollars on STR platforms in Toronto. 
...
(ref:fig-1-5) _The percentage of active listings and revenue accounted for by multilistings inToronto (14-day average)_
