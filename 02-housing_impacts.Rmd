...
The `r housing_loss_2019` housing units taken off of Toronto’s housing market in 2019 are `r housing_loss_pct_dwelling` of the total amount of housing in the city, and this housing loss has been concentrated in small parts of the city. This is more or less the same in relative to the total housing stock: Figure \@ref(fig:fig-2-3) shows the proportion of each ward or dissemination area’s housing stock which was operated as a dedicated short-term rental as of the end of 2019. The maps show a tale of two cities: in most of Toronto, there are relatively few dedicated STRs, while in the center of the city as well as Spadina-Fort York, they are ubiquitous. In the Spadina-Fort York ward, `r housing_loss_pct_spadina` of all housing units have been converted to dedicated STRs. The figure is `r housing_loss_pct_university` for University-Rosedale and `r housing_loss_pct_centre` for Toronto Centre. In Spadina-Fort York, the rental vacancy rate was `r vacancy_spadina` in 2019. This means that there are more dedicated STRs in this neighbourhood than there are vacant apartments for rent.
...
# Highest WD vacancy rate
highest_vacancy_WD_rate <- 
  annual_vacancy %>% 
  filter(date == 2019, dwelling_type == "Total", bedroom == "Total", 
         quality == "a") %>% 
  filter(vacancy == max(vacancy)) %>% 
...
# Highest WD vacancy rental units
highest_vacancy_WD_apts <- 
  annual_units %>% 
  filter(zone_name == highest_vacancy_WD, 
...
# Highest WD vacancy vacant rental units
highest_vacancy_WD_vac_apts <- 
    annual_units %>% 
  filter(zone_name == highest_vacancy_WD_apts, 
         date == 2019, dwelling_type == "Total", 
         bedroom == "Total") %>% 
  pull(units) %>%
  {. * parse_number(highest_vacancy_WD_rate) / 100} %>% 
  round(-2) %>% 
  prettyNum(",")

# 3+ bedroom vacancy rate Spadina-Fort York
spadina_vac_3_bed <- 
  annual_vacancy %>% 
  filter(zone_name == "Spadina-Fort York", date == 2019, dwelling_type == "Total",
         bedroom == "3 Bedroom +") %>% 
  pull(vacancy) %>% 
  scales::percent(0.1)
