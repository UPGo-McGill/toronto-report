...
highest_vacancy_zone_vac_apts <- 
    annual_units %>% 
  filter(zone_name == highest_vacancy_zone_apts, 
         date == 2019, dwelling_type == "Total", 
         bedroom == "Total") %>% 
  pull(units) %>%
...
In 2019, the City of Toronto’s rental vacancy rate stood at only `r city_vacancy_2019`—a crisis-level rate, albeit slightly higher than the previous few years. The standard rule of thumb is that a healthy rental market should have a vacancy rate of at least 3%, and all 10 of the CMHC's zones in Toronto are substantially below that. `r highest_vacancy_WD` has the highest vacancy rate at only `r highest_vacancy_WD_rate`, which means that, of the zone’s approximately `r highest_vacancy_WD_apts` rental apartments, fewer than `r highest_vacancy_WD_vac_apts` were available to be rented by prospective tenants in October 2019, when CMHC’s survey was conducted. In general, vacancy rates are even lower for family-sized housing units (defined by CMHC as units with two or more bedrooms). For example, the vacancy rate for units with three or more bedrooms was `r spadina_vac_3_bed` for the Spadina-Fort York zone.
...
Between 2018 and 2019, the City of Toronto as a whole saw an `r total_unit_change`-unit increase, more than half of which occurred in Southeast Toronto. In a zone like Spadina-Fort York, where there is a high concentration of STR operations and a growth in housing loss, the zone’s net rental housing change is greatly affected by new STR conversions in 2019. In almost every single of the ten CMHC zones (shown in Figure \@ref(fig:fig-2-4)), new STR conversions in 2019 either mostly cancelled out new rental housing production, or substantially decreased the net rental housing supply from 2018 to 2019.
