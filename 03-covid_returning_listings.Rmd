# STRs returning to the long-term market
...
**Using image recognition techniques, we identified 3,476 unique Airbnb listings which were posted as long-term rentals (LTRs) on Craigslist or Kijiji between March and December 2020. These former STRs have asking rents on average 20.9% higher than other LTR listings, but are correlated with a 14.8% decrease in overall asking rents in Toronto. The evidence suggests that the overwhelming majority of STR listings transferred to LTR platforms are commercial operations. We estimate that 42.2% have fully transitioned back to the long-term market, 23.6% have been temporarily blocked on Airbnb and may return to being STRs in the future, and 34.1% failed to be rented on LTR platforms and instead remain active on Airbnb. This latter figure can be broken down in two: 22.8% have a new minimum stay of 28 nights or more to comply with the new regulation, while 11.3% continue to operate on the shorter-term side on STR platforms.**
...
first_photo_pair <-
  cl_matches %>%
  filter(match == "match") %>%
  filter(field(x_sig, "file") == 
           "/Volumes/Data 2/Scrape photos/toronto/ab/ab-14685177-1.jpg") %>% 
  slice(1)
...
cl_matches %>%
  filter(match == "match") %>%
  filter(field(x_sig, "file") == 
           "/Volumes/Data 2/Scrape photos/toronto/ab/ab-36199496-1.jpg") %>% 
  slice(1)

titles <- list(
  
  vctrs::field(first_photo_pair$x_sig, "file") %>%
    str_extract('ab-.*(?=-[:digit:]*\\.jpg)') %>%
    {filter(property, property_ID == .)} %>%
    pull(listing_title),
  
  vctrs::field(first_photo_pair$y_sig, "file") %>%
  ...
  vctrs::field(second_photo_pair$x_sig, "file") %>%
    str_extract('ab-.*(?=-[:digit:]*\\.jpg)') %>%
    {filter(property, property_ID == .)} %>%
    pull(listing_title),
  
  vctrs::field(second_photo_pair$y_sig, "file") %>%
    str_extract('cl-.*(?=-[:digit:]*\\.jpg)') %>%
    {filter(ltr, id == .)} %>%
    slice(1) %>%
    pull(title) %>%
...
figure_6_1_fun <- function(regular = "", condensed = "") {
  ...
  pmap(list(list(field(first_photo_pair$x_sig, "file"),
                 field(first_photo_pair$y_sig, "file"),
                 field(second_photo_pair$x_sig, "file"),
                 field(second_photo_pair$y_sig, "file")),
              titles, c("A", "B", "C", "D")),
         ~{.x %>%
             load.image() %>%
...
The COVID-19 pandemic has caused an unprecedented decline in STR activity in Toronto. More recently, the impending imposition of the City's STR by-law may also have contributed to a decline in STR activity. Under these circumstances, it would be reasonable to imagine that some STR hosts—particularly commercial operators who had come to expect large income streams from their properties—may have decided to return their listings to the long-term housing market, either temporarily or permanently. To investigate this possibility, we collected listing images from all properties posted to Craigslist and Kijiji in Toronto between April 2020 and January 2021, and used image recognition analysis to match STR listings on Airbnb and Vrbo to long-term rental (LTR) listings on Craigslist and Kijiji. These latter two platforms represent only a portion of the LTR market, but provide useful insight into how STR hosts have responded to the collapse in accommodation demand during the COVID-19 pandemic, or how they expected the regulation to impact their businesses. If the exact same photo of an apartment’s living room was uploaded to Airbnb in August 2019 and then to Craigslist in April 2020, this provides proof that the property in question has moved from the STR market to the LTR market. The image recognition software we developed is able to identify matches between images which are identical, but also images which the host has modified slightly, and thus allows us to reliably identify every match which exists between STR and LTR platforms (Figure \@ref(fig:fig-6-1)).
...
(ref:fig-6-1) _Examples of listings matched between STR and LTR platforms. Each pair of images is matched despite one of each pair having text overlays._
...
property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  count(active = active >= start_2020 | created >= start_2020) %>% 
  mutate(pct = n / sum(n))
...
Our image matching algorithm recognized `r unique_str_matches` unique Airbnb listings which matched with `r unique_ltr_matches` different LTR listings (as some units are posted multiple times) in the City of Toronto. The matching LTR listings were equally found on Kijiji (`r unique_KJ_matches` listings, or `r unique_KJ_pct`) and Craigslist (`r unique_CL_matches` listings, or `r unique_CL_pct`). Out of the `r unique_str_matches` matching Airbnb listings, `r str_matches_still_active_pct` (`r str_matches_still_active` listings) were created or still active in 2020. We suspect that many or most of the remaining properties were also still active under a different listing ID and with a different photo, since commercial STRs are delisted and relisted quite frequently, and we thus consider the `r unique_str_matches` Airbnb listings which we matched to Craigslist and Kijiji to be a lower bound for the number of unique housing units that went from the STR market to the LTR market due to the COVID-19 pandemic and the expectations of the new regulation. (Each listing which we matched is guaranteed to have been listed first on Airbnb and then on either Kijiji or Craigslist, but there are certain to be additional listings which we did not match because they did not reuse the same photographs.) 

All of the Craigslist listings we matched were long-term rentals. Out of the `r kj_matches` Airbnb listings which we matched to Kijiji, `r kj_matches_ltr_pct` were identified by their hosts as “long-term rentals” and `r kj_matches_str_pct` were identified as “short-term rentals” (a category which typically indicates month-to-month leases). Among matched Kijiji listings, `r kj_lease_one_year` specified lease lengths of one year, `r kj_lease_month` specified month-to-month, and `r kj_lease_NA` did not specify.
...
figure_6_2_fun <- function(regular = "", condensed = "") {
  ...
  first_ltr_listing %>% 
  group_by(kj) %>% 
  mutate(n = slider::slide_dbl(n, mean, .before = 2)) %>% 
  ungroup() %>% 
...
The first COVID-19 case in Ontario was confirmed on January 25, 2020, but the pandemic did not fully erupt until the second week of March 2020, when public facilities and private businesses began to close, culminating in a Provincial declaration of public emergency on March 17. Consistent with this timeline, what was in early March a trickle of Airbnb listings moving to Craigslist or Kijiji began to accelerate in the second half of the month (Figure \@ref(fig:fig-6-2)). By the end of the March, the number of daily transfers reached `r march_peak_listings_transfer` listings. Daily numbers remained high through April (its peak being `r peak_listing_transfer` listings transferred on April 21), but even from May through December an average of `r avg_transfer_may_dec` new Airbnb listings were transferred to Craigslist or Kijiji each day. It is likely, although there is no way to be sure, that the knowledge among commercial STR hosts that their listings would be illegal when the City's new by-law took effect on 1 January 2021 contributed to the transfer of STR units to the long-term market in the latter months of 2020. 
...
(ref:fig-6-2) _The date on which matched Airbnb listings were first detected on Craigslist or Kijiji in the City of Toronto (3-day average)_
...
waterfront_str_match_pct <-
  matches_by_ward %>%
  filter(ward == "Spadina-Fort York") %>%
  pull(pct) %>%
  scales::percent(0.1)

toronto_center_str_match_pct <-
  matches_by_ward %>%
  filter(ward == "Toronto Centre") %>%
  pull(pct) %>%
  scales::percent(0.1)

uni_rosedale_match_pct <-
  matches_by_ward %>%
...
waterfront_match_as_pct_mar <-
  ...
  pull(n)} /
  {daily %>%
      filter(housing, ward == "Spadina-Fort York", status != "B",
             date == "2020-03-01") %>%
      nrow}} %>%
  scales::percent(0.1)

waterfront_match_as_pct_2020 <-
  {{property %>%
    st_drop_geometry() %>%
    filter(!is.na(ltr_ID)) %>%
    count(ward) %>%
...
toronto_centre_match_as_pct_mar <-
  {{property %>%
  st_drop_geometry() %>%
  filter(!is.na(ltr_ID)) %>%
  count(ward) %>%
  ...
  pull(n)} /
  {daily %>%
      filter(housing, ward == "Toronto Centre", status != "B",
             date == "2020-03-01") %>%
      nrow}} %>%
  scales::percent(0.1)

toronto_centre_match_as_pct_2020 <-
  {{property %>%
    st_drop_geometry() %>%
    filter(!is.na(ltr_ID)) %>%
    count(ward) %>%
...
Out of the `r unique_str_matches` unique STR listings matched to LTR listings in the City of Toronto, nearly half (`r waterfront_str_match_pct`) were located in the Spadina-Fort York ward, with the remaining matches more evenly split between the other wards. `r toronto_center_str_match_pct` of matches were in the Toronto Centre ward, followed by `r uni_rosedale_match_pct` in University-Rosedale. The rest of the wards were below 6% each. Even accounting for the general concentration of STRs in Spadina-Fort York, this ward is still highly over-represented in these matches (Figure \@ref(fig:fig-6-3)). Toronto Centre is likewise over-represented. The number of STR listings matched to LTR listings in Spadina-Fort York is equivalent to more than a third (`r waterfront_match_as_pct_mar`) of all the STR listings active in the ward on March 1, 2020, and more than a fifth `r waterfront_match_as_pct_2020` of all the listings active in the ward in 2020. In Toronto Centre, the matches found represent `r toronto_centre_match_as_pct_mar` of all active STR listings on March 1, and `r waterfront_match_as_pct_2020` of all active listings in 2020.
...
(ref:fig-6-3) _Total number of STR listings matched to LTR listings (L), and matched STR listings as a percentage of daily active listings on 2020-03-01 (R), by ward_
...
asking_rents <- 
  ...
  mutate(avg_price = slider::slide_dbl(avg_price, mean, .before = 6)) %>% 
  ungroup() %>% 
...
asking_rent_apr <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-04-01",
         created <= "2020-04-30") %>% 
  group_by(created) %>% 
...
asking_rent_apr_city <- 
  asking_rent_apr %>% 
  summarize(city = mean(city)) %>% 
  pull(city) %>% 
  scales::dollar(10)

asking_rent_apr_match <- 
  asking_rent_apr %>% 
  summarize(match = mean(match)) %>% 
  pull(match) %>% 
  scales::dollar(10)

asking_rent_apr_dif_pct <- 
  {parse_number(asking_rent_apr_match) / parse_number(asking_rent_apr_city) - 1} %>% 
  scales::percent(0.1)

asking_rent_dec <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-12-01",
         created <= "2020-12-31") %>% 
  group_by(created) %>% 
  summarize(city = avg_price[status == "All listings"],
            match = avg_price[status == "Matched to STR"],
...
asking_rent_dec_city <- 
  asking_rent_dec %>% 
  summarize(city = mean(city)) %>% 
  pull(city) %>% 
  scales::dollar(10)

asking_rent_dec_match <- 
  asking_rent_dec %>% 
  summarize(match = mean(match)) %>% 
  pull(match) %>% 
  scales::dollar(10)

asking_rent_dec_dif_pct <- 
  {parse_number(asking_rent_dec_match) / parse_number(asking_rent_dec_city) - 1} %>% 
  scales::percent(0.1)
...
rent_dif_apr_dec_waterfront <- 
  asking_rents %>% 
  filter(geography == "Spadina-Fort York", created >= "2020-04-01", 
...
asking_rent_early_apr <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-04-01", 
         created <= "2020-04-14", status == "All listings") %>% 
...
asking_rent_dec_end <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-12-16", 
         created <= "2020-12-31", status == "All listings") %>% 
  summarize(avg = mean(avg_price)) %>% 
  pull(avg) %>% 
  scales::dollar(1)
...
rent_decline_pct <- 
  {parse_number(asking_rent_early_apr) / parse_number(asking_rent_dec_end) - 1} %>% 
  scales::percent(0.1)
...
figure_6_4_fun <- function(regular = "", condensed = "") {
  ...
  asking_rents %>% 
    group_by(geography, status) %>% 
    mutate(avg_price = slider::slide_dbl(avg_price, mean, .before = 13)) %>% 
    ungroup() %>%
    filter(created >= "2020-04-01", created <= "2020-12-31") %>% 
    ggplot(aes(created, avg_price, color = status)) +
    geom_line(lwd = 1) +
    scale_x_date(name = NULL, limits = c(as.Date("2020-04-01"), NA)) +
    scale_y_continuous(name = NULL, limits = c(1500, 3000), 
                       label = scales::dollar) +
...
The left panel of Figure \@ref(fig:fig-6-4) shows the average asking rents of listings posted to Craigslist and Kijiji between April and December 2020. The asking rents of the STR-LTR matches have remained significantly higher than non-matched LTR listings. In April 2020, when the average asking rent on LTR platforms in the City of Toronto was `r asking_rent_apr_city`, the average asking rent among listings which we matched to Airbnb was `r asking_rent_apr_match`— `r asking_rent_apr_dif_pct` higher. Over the course of the spring and the rest of the year, average asking rents for LTR listings matched to Airbnb declined continuously. In December, the average asking rent was `r asking_rent_dec_city` for all Craigslist and Kijiji listings and `r asking_rent_dec_match` for the matches. Overall, from April to December, asking rents for properties matched to STR listings were `r rent_dif_apr_dec` higher than the city-wide average.
...
(ref:fig-6-4) _Average asking rents on Craigslist and Kijiji in the City of Toronto (L) and the waterfront (R) (7-day average)_

The right panel of Figure \@ref(fig:fig-6-4) shows asking rents only in the Spadina-Fort York ward. A large portion of the divergence in asking rents between LTR listings matched to Airbnb and listings not matched is a compositional effect of the much greater frequency of a Spadina-Fort York location (which commands higher prices than the rest of the city) among matched listings. Even in this ward, however, LTR listings matched to STRs have been on average `r rent_dif_apr_dec_waterfront` more expensive than listings not matched. Overall, STRs returning to the long-term market are correlated with a significant decline in asking rents. The average city-wide asking rent on Craigslist and Kijiji declined `r rent_decline_pct` from `r asking_rent_early_apr` in early April 2020 to `r asking_rent_dec_end` in late December 2020.

```{r bedrooms, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "ltr_processed.qsm"))}
...
bedroom_match_table <-
  ...
  select(bedrooms, pct) %>% 
  mutate(Category = "City of Toronto rental stock", .before = bedrooms)

bedroom_match_table <-
  property %>% 
  st_drop_geometry() %>% 
  filter(housing, active >= start_2020, created <= "2020-12-31") %>% 
  count(bedrooms) %>% 
...
match_2br_pct <- 
  bedroom_match_table %>% 
  filter(Category == "LTRs matched to STR") %>% 
  pull(`2-bedroom`) %>% 
  scales::percent(0.1)

nonmatch_2br_pct <- 
  bedroom_match_table %>% 
  filter(Category == "LTRs not matched") %>% 
  pull(`2-bedroom`) %>% 
  scales::percent(0.1)

str_2br_pct <- 
  bedroom_match_table %>% 
  filter(Category == "STR market (2020)") %>% 
  pull(`2-bedroom`) %>% 
  scales::percent(0.1)

city_2br_pct <- 
  bedroom_match_table %>% 
  filter(Category == "City of Toronto rental stock") %>% 
  pull(`2-bedroom`) %>% 
  scales::percent(0.1)
...
match_eh_commercial_2020_transfer_pct <- 
  {{property %>% 
    ...
    filter(listing_type == "Entire home/apt", status != "B", 
           date >= start_2020, (FREH_3 > 0.5 | multi == TRUE)) %>% 
    count(property_ID) %>% 
    nrow()}} %>% 
  scales::percent(0.1)
...
Figure \@ref(fig:fig-6-5) shows the age in years of STR listings which matched a LTR platform (left panel) and which did not match (right panel). The age distribution of matched listings is sligthly skewed to the left, which means that a larger proportion of matched listings are less than a year old. By contrast, STR listings which did not match tend to be slightly older. This pattern suggests that newer commercial STR listings may have been in a weaker financial position at the onset of the pandemic, prompting their hosts to change strategies more rapidly than established hosts.
...
matches_present_table <- 
  ...
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020) %>% 
  summarize(total = n(),
            total_pct = total / nrow(filter(property, !is.na(ltr_ID))),
            n_scraped = sum(scraped >= "2020-01-23"),
            pct_scraped = mean(scraped >= "2020-01-23"),
...
furnished_matches_pct <- 
  ...
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020, scraped < "2020-01-23") %>% 
...
matches_active_jan_and_dec <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-01-23") %>% 
  summarize(total = n(),
            n_inactive = total - sum(active >= max(active, na.rm = T) - days(30), na.rm = TRUE) +
              sum(is.na(active)),
            pct_inactive = n_inactive / total) %>% 
  pull(total)

matches_active_jan_and_dec_blocked <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-01-23") %>% 
  summarize(total = n(),
...
matches_active_jan_and_dec_blocked_pct <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-01-23") %>% 
  summarize(total = n(),
...
matches_active_jan_and_dec_lt <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-01-23") %>% 
  summarize(total = n(),
            n_lt = sum(active > max(active, na.rm = T) - days(30) & minimum_stay >= 28, na.rm = TRUE),
            pct_lt = n_lt / total) %>% 
  pull(n_lt)

matches_active_jan_and_dec_lt_pct <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-01-23") %>% 
  summarize(total = n(),
...
deactivated_pct <- 
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020) %>% 
  summarize(pct_not_scraped = 1 - mean(scraped >= max(active, na.rm = T) - days(30))) %>% 
...
blocked_pct <-
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020) %>% 
  summarize(pct_blocked = (sum(active < max(active, na.rm = T) - days(30) & scraped >= max(scraped, na.rm = T) - months(1), 
                               na.rm = TRUE) + sum(is.na(active) & scraped >= 
                                                     max(scraped, na.rm = T) - months(1))) / n()) %>% 
  pull(pct_blocked)
...
active_lt_pct <-
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020) %>% 
  summarize(pct_lt_active = sum(active > max(active, na.rm = T) - days(30) & minimum_stay >= 28, na.rm = TRUE) /n()) %>% 
  pull(pct_lt_active)
...
total_table <- 
  ...
  summarize(total = n(),
            ...
            active_28 = total * active_lt_pct,
            active = total - deactivated - blocked - active_28) %>% 
...
Further evidence about the extent to which STR hosts are successfully transferring their listings to the long-term market can be found by examining the activity of the STR listings themselves. Are hosts planning on renting on the long-term only temporarily, leaving the STR listing running for future bookings? Or have they deactivated the STR listing? Out of the total `r unique_str_matches` STR listings which we identified on LTR platforms, `r matches_present_jan` (`r matches_present_jan_pct`) were present on Airbnb or Vrbo at the beginning of 2020. Out of this number, `r matches_present_dec` (`r matches_present_dec_pct`) were still present on 31 December 2020, while the other `r matches_deactivated` (`r matches_deactivated_pct`) had been deactivated. Extrapolating this proportion across the entire set of matched listings we identified, we estimate that `r round(extrapolated_deactivated)` matched listings have been deactivated from Airbnb or Vrbo during the pandemic, while `r round(extrapolated_active)` remain on the platforms. Listings removed from STR platforms are likely to have been durably shifted to the long-term market. However, `r furnished_matches_pct` of these listings were rented as furnished rentals on Craigslist or Kijiji. These listings should therefore be considered at relatively high risk of returning to the STR market if demand recovers.

Listings which remain on Airbnb and Vrbo can nevertheless be inactive. If a host successfully rents their listing on Craigslist for several months, they can choose to block their calendar on their STR platform to make sure no new reservations occur, while keeping the listing intact for when the STR market recovers. Of the `r matches_active_jan_and_dec` matched listings present on Airbnb or Vrbo at the beginning of 2020 and still present by the end of December, `r matches_active_jan_and_dec_blocked` (`r matches_active_jan_and_dec_blocked_pct`) were blocked for the entirety of the month of December. This suggests that these listings are not active on the STR market and therefore have been rented on the LTR market, but the fact that the STR listings have not been taken down suggests that the hosts may plan to reactivate their units once STR demand recovers. Furthermore, `r matches_active_jan_and_dec_lt` (`r matches_active_jan_and_dec_lt_pct`) of the matched listings which failed to transfer to the LTR market have set their minimum stay at 28 days in order to remain on Airbnb but not need to register with the City. 

In total, taking into account the matched listings which have continued to see activity on Airbnb, we estimate that, of the total `r unique_str_matches` STR listings which were advertised on Craigslist or Kijiji, `r total_deactivated` (`r total_deactivated_pct`) have been deactivated from Airbnb and Vrbo and have likely transitioned back to long-term housing (albeit often as furnished rentals which could be reconverted to STRs). `r total_blocked` (`r total_blocked_pct`) have been temporarily blocked on the STR platforms and have likely been rented in the long-term market but may return to being STRs in the future. `r total_active_28` (`r total_active_28_pct`) are still operating on Airbnb but with a 28-day minimum stay. `r total_active` (`r total_active_pct`) failed to be rented on LTR platforms and instead remain active as traditional short-term rentals (minimum stay of less than 28 days) on Airbnb.
