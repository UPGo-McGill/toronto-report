...
FREH_10 <-
  ...
  filter(date >= max(date, na.rm = T) - months(1), FREH_3 >= 0.5) %>%
...
multi_10 <- 
  daily %>% 
  filter(property_ID %in% january_listings$property_ID) %>% 
  filter(date >= max(date, na.rm = T) - months(1), multi) %>%
  distinct(property_ID, .keep_all = TRUE) %>% 
...
license_scrape <- 
  ...
  mutate(
    status_10 = case_when(
      active < max(active, na.rm = T) - months(1) ~ "Inactive",
      FREH_10 ~ "FREH",
...
eh_multiple_license <- 
  license_scrape %>% 
  st_drop_geometry() %>% 
  filter(license_status == "Duplicates", active >= max(active, na.rm = T) - months(1)) %>% 
...
(ref:fig-3-3) _License status and listing status (in October 2020) for listings displayed on Airbnb in end of January 2021_
