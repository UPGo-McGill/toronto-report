...
active_listings_2021 <- 
  property %>% 
  st_drop_geometry() %>% 
  count(scraped) %>% 
  filter(scraped >= "2020-11-01") %>% 
  mutate(cum_n = slider::slide_dbl(n, sum, .after = 100)) %>% 
  filter(scraped <= "2021-01-24")

rm(property, daily, GH)

```

As of 1 January 2021, all STR operators in Toronto are required to have a municipal license. This means that all properties displayed on Airbnb (with the exception of properties rented exclusively for 30 days at a time or longer, and hotels and other tourist accommodations which are regulated separately) must have a license visible on Airbnb.ca. (Vrbo, Airbnb's biggest competitor recently pulled out of the Toronto market.) Our team scraped the Airbnb website on five occasions between January 7 and February 5 to record all publicly visible registration numbers for Toronto STRs. We have connected that information with the City's Short Term Rentals Registration database and with our own data on STR activity in order to assess regulatory compliance. 

``` {r make_fig_3_1}

figure_3_1_fun <- function(regular = "", condensed = "") {
  
  ggplot(active_listings_2021) +
    geom_line(aes(scraped, cum_n), color = col_palette[1]) +
...
Figure \@ref(fig:fig-3-1) shows the total number of active listings that were displayed on the Airbnb platform from 1 November 2020 to 24 January 2021 (the last date for which we have complete information). It demonstrates a sharp drop of more than 2,600 listings around January 11, which appears to have been a mass removal initiated by Airbnb of listings without licenses.
...
(ref:fig-3-1) _Displayed listings on Airbnb, 1 November 2020 - 24 January 2021_
