...
asking_rent_dec <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-10-25", 
...
matches_present_table <- 
  ...
  summarize(total = n(),
            ...
            n_scraped = sum(scraped >= "2020-10-31"),
            pct_scraped = mean(scraped >= "2020-10-31"),
...
furnished_matches_pct <- 
  ...
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01", scraped < "2020-10-31") %>% 
...
matches_active_jan_and_oct <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= "2020-10-31") %>% 
  summarize(total = n(),
...
deactivated_pct <- 
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01") %>% 
  summarize(pct_not_scraped = 1 - mean(scraped >= "2020-10-01")) %>% 
...
blocked_pct <-
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01") %>% 
  summarize(pct_blocked = (sum(active < "2020-10-01" & scraped >= "2020-10-01", 
