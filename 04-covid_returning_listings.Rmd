# Covid-19 and Regulations Expectations: STRs returning to the long-term market
...
#   filter(x_name == "/Volumes/Data 2/Scrape photos/Toronto/ab/ab-18753643.jpg")
# 
# second_photo_pair <-
#   cl_matches %>%
#   filter(confirmation == "match") %>%
#   filter(x_name == "/Volumes/Data 2/Scrape photos/Toronto/ab/ab-10972081.jpg")
# 
# titles <- list(
# 
...
As demonstrated in the first section, the COVID-19 pandemic has caused an unprecedented decline in STR activity in Toronto. More recently, the expectations of the new regulation has also contributed to this decline in activity. Under these circumstances, it would be reasonable to imagine that some STR hosts—particularly commercial operators who had come to expect large income streams from their properties—may have decided to return their listings to the long-term housing market, either temporarily or permanently. To investigate this possibility, we collected listing images from all properties posted to Craigslist and Kijiji in Toronto between March and December 2020, and used image recognition analysis to match STR listings on Airbnb and Vrbo to long-term rental (LTR) listings on Craigslist and Kijiji. These latter two platforms represent only a portion of the LTR market, but provide useful insight into how STR hosts have responded to the collapse in accommodation demand during the COVID-19 pandemic, or how they expected the regulation to impact their businesses. If the exact same photo of an apartment’s living room was uploaded to Airbnb in August 2019 and then to Craigslist in April 2020, this provides proof that the property in question has moved from the STR market to the LTR market. The image recognition software we developed is able to identify matches between images which are identical, but also images which the host has modified slightly, and thus allows us to reliably identify every match which exists between STR and LTR platforms 
<!-- (Figure \@ref(fig:fig-6-1)). -->
...
The first COVID-19 case in Ontario (and Canada) was confirmed on January 25, 2020, but the pandemic did not fully erupt until the second week of March 2020, when public facilities and private businesses began to close, culminating in a Provincial declaration of public emergency on March 17. Consistent with this timeline, what was in early March a trickle of Airbnb listings moving to Craigslist or Kijiji began to accelerate in the middle of the month (Figure \@ref(fig:fig-6-2)). By the end of the March, the number of daily transfers reached `r march_peak_listings_transfer` listings. Daily numbers remained high through April (its peak being `r peak_listing_transfer` listings transferred on April 21), but even from May through December an average of `r avg_transfer_may_dec` new Airbnb listings were transferred to Craigslist or Kijiji each day. The expectations of the new regulation starting in January 2021 probably also contributed to this transfer in the latest months of the year 2020. 
...
asking_rent_mar_13 <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-03-13",
         created <= "2020-04-30") %>% 
  group_by(created) %>% 
...
asking_rent_dec_31 <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created == "2020-12-31") %>% 
  filter(status == "All listings") %>% 
  pull(avg_price) %>% 
  scales::dollar(1)

asking_rent_dec_31_match <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created == "2020-12-31") %>% 
...
rent_dif_apr_dec_waterfront <- 
  asking_rents %>% 
  filter(geography == "Spadina-Fort York", created >= "2020-03-13", 
...
rent_decline_pct <- 
  asking_rents %>% 
  filter(geography == "City of Toronto" & status == "All listings" &
           ((created >= "2020-03-25" & created <= "2020-03-31") | 
           (created >= "2020-12-01" & created <= "2020-12-31"))) %>% 
  group_by(dec = created >= "2020-12-01") %>% 
...
asking_rent_mar <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-03-25", 
...
asking_rent_dec <- 
  asking_rents %>% 
  filter(geography == "City of Toronto", created >= "2020-12-25", 
...
The left panel of Figure \@ref(fig:fig-6-4) shows the average asking rents of listings posted to Craigslist and Kijiji between March and December 2020, but since we do not have complete asking rent data for August and September, those months are excluded. The asking rents of the STR-LTR matches have remained dramatically higher than non-matched LTR listings. On March 13, when the average asking rent on LTR platforms in the City of Toronto was `r asking_rent_mar_13`, the average asking rent among listings which we matched to Airbnb was `r asking_rent_mar_13_match`— `r asking_rent_mar_13_match_dif_pct` higher. Over the course of the spring and the rest of the year, average asking rents for LTR listings matched to Airbnb declined continously. At the end of December, the average asking rent was `r asking_rent_dec_31` for all Craigslist and Kijiji listings and `r asking_rent_dec_31_match` for the matches. Overall, from April to December, asking rents for properties matched to STR listings were `r rent_dif_apr_dec` higher than the city-wide average.
...
_Size:_ Table \@ref(tab:tab-bedrooms) shows the distribution of units by number of bedrooms for entire-home STRs, as well as all the units that were posted for rent on LTR platforms (both the ones that matched with an STR listing and the ones that did not match), and for the City of Toronto’s overall primary (i.e. apartment and townhouse) rental housing stock. Two-bedrooms were overrepresented in both the matches (`r match_2br_pct`) and the non-matches (`r nonmatch_2br_pct`) compared to the STR market (`r str_2br_pct`), but significantly underrepresented compared to the City (`r city_2br_pct`). Studios were overrepresented among LTR listings which matched to Airbnb or Vrbo (`r match_studio_pct`) compared with LTR listings which did not match (`r nonmatch_studio_pct`) and STR listings (`r str_studio_pct`), but there isn't a big variation when compared to Toronto’s rental stock of studios (`r city_studio_pct`). Units with three bedrooms on the STR market and for matches were similar to the City distribution, while one-bedroom units were significantly overrepresented on STR platforms compared to LTR platforms and to the City's primary rental stock. The implication is that two-bedrooms and studios apartments were disproportionately likely to be moved from STR platforms to the long-term rental market, while one-bedroom apartments were disproportionately likely not to have been moved.

``` {r tab-bedrooms, include = TRUE}

library(kableExtra)

bedroom_match_table %>% 
  mutate(across(where(is.numeric), scales::percent, 0.1)) %>%  
  kbl(caption = "STR and LTR listings by bedroom count", align = "lrrrr") %>% 
  kable_styling()
...
_Furnished vs. unfurnished:_ To accommodate temporary guests, STR properties are overwhelmingly furnished. Unsurprisingly, properties that have moved from the STR to LTR market during the pandemic are listed as furnished at much higher rates than other listings on Craigslist and Kijiji. Of the `r total_ltr_listings` LTR listings we analyzed, `r furnished_pct` were listed as furnished and `r unfurnished_pct` as unfurnished. (The remainder did not provide this information.) Listings which matched with STRs had a significantly higher proportion classified as furnished: `r furnished_match_pct` furnished and `r unfurnished_match_pct` unfurnished. These proportions suggest that, in almost 6 out of 10 cases, STR operators listing their units on LTR platforms may intend to return their units to the STR market when demand returns, while in 4 cases out of 10, the move to the LTR market appears to be reasonably permanent, since the host has gone through the effort of emptying the unit of furniture.
...
Nearly all of the STR listings which we matched to LTR listings on Craigslist or Kijiji are likely to have been dedicated, commercial STRs. This is because it is highly unlikely that a casual home sharer operating a STR out of their principal residence would decide to vacate their home, list it on Craigslist or Kijiji, and reuse the photo they had used to advertise the short-term rental. It is therefore also likely that the STR listings found on LTR platform were non-compliant with the City's STR upcoming regulations, since the fact that they were commercial operators makes it extremely difficult to also have the listing be operated out of a principal residence. We can test this intuition by examining the characteristics of the STR listings which we matched to an LTR platform.
...
matches_present_table <- 
  ...
  summarize(total = n(),
            ...
            n_scraped = sum(scraped >= max(scraped) - months(1)),
            pct_scraped = mean(scraped >= max(scraped) - months(1)),
...
furnished_matches_pct <- 
  ...
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= start_2020, scraped < max(scraped) - months(1)) %>% 
...
matches_active_jan_and_dec <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= max(scraped) - months(1)) %>% 
  summarize(total = n(),
