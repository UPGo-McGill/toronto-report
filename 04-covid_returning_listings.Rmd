...
**Using image recognition techniques, we identified 3,324 unique Airbnb listings which were posted as long-term rentals (LTRs) on Craigslist or Kijiji between March and September 2020. These former STRs have asking rents on average 21.5% higher than other LTR listings, but are correlated with a 14.7% decrease in overall asking rents in Toronto. The evidence suggests that the overwhelming majority of STR listings transferred to LTR platforms are commercial operations. We estimate that 35.2% have fully transitioned back to the long-term market, 31.3% have been temporarily blocked on Airbnb and may return to being STRs in the future, and 33.5% failed to be rented on LTR platforms and instead remain active on Airbnb.**
...
furnished_matches_pct <- 
  ...
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01", scraped < max(scraped)-months(1)) %>% 
...
matches_active_jan_and_oct <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= max(scraped)-months(1)) %>% 
  summarize(total = n(),
            n_inactive = total - sum(active >= "2020-10-01", na.rm = TRUE) +
              sum(is.na(active)),
            pct_inactive = n_inactive / total) %>% 
  pull(total)

matches_active_jan_and_oct_blocked <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= max(scraped)-months(1)) %>% 
  summarize(total = n(),
            n_inactive = total - sum(active >= max(active)-months(1), na.rm = TRUE) +
              sum(is.na(active)),
            pct_inactive = n_inactive / total) %>% 
  pull(n_inactive)

matches_active_jan_and_oct_blocked_pct <- 
  property %>% 
  st_drop_geometry() %>% 
  filter(!is.na(ltr_ID), scraped >= max(scraped)-months(1)) %>% 
  summarize(total = n(),
...
deactivated_pct <- 
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01") %>% 
  summarize(pct_not_scraped = 1 - mean(scraped >= max(scraped)-months(1))) %>% 
...
blocked_pct <-
  ...
  filter(!is.na(ltr_ID)) %>% 
  filter(scraped >= "2020-01-01") %>% 
  summarize(pct_blocked = (sum(active < max(active)-months(1) & scraped >= max(scraped)-months(1), 
