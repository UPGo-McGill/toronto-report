...
property_status <- 
  ...
  mutate(status = case_when(scraped <= max(scraped)-months(2) ~ "Taken down", # Taken down before end of october
                            active < "2020-10-01" ~ "Blocked", # Not active during all october
...
STR listings placed on the LTR market have had a significant higher monthly revenue in 2020. Indeed, while the non-matched listings have made a median revenue (the dashed lines on Figure \@ref(fig:fig-5-1)) of \$`r monthly_rev_median_ltr[1,2]`, the listings that did match have made $`r monthly_rev_median_ltr[2,2]`. This is even more true when looking at the median monthly revenue of the precedent year for the same listings, where they have respectively made \$`r monthly_rev_median_ltr[1,3]` and \$`r monthly_rev_median_ltr[2,3]`. The drop in revenue between the two years, for both group, is also notable. Figure \@ref(fig:fig-5-1) demonstrates how the STR listings that did not match a LTR listing have had a significant lower monthly revenue in 2020. It is in line with our previous assessment, stating that it is the most affluent hosts which choose to move their lucrative units on the LTR market.
...
Monthly revenues in 2020 may not be the only relevant factor to consider when looking to explain the behavior of a host moving his STR unit to the LTR market. Indeed, it may be useful to see the relative loss of monthly revenues between 2019 and 2020, again separated between the two groups previously evaluated. While the vast majority of listings had a decrease in monthly revenue, it is the listings matched on the LTR that have had a worse relative loss in monthly revenue, as Figure \@ref(fig:fig-5-2) shows. In brief, listings found on the LTR market made a higher monthly revenue in 2020 than the other listings, but had a greater decrease in monthly revenue when compared to 2019's monthly revenues levels.
